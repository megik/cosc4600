...
public class StateNode {
	public static final Integer[] GOAL = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
	...
	public StateNode(Integer[] initState, Integer[] currState, int cost, String actionTaken, StateNode predecssor,
			int emptyPos) {
	...
	public StateNode(StateNode node) {
		this(node.getInitialState(), node.getCurrentState(), node.getCost(), node.getAction(), node.getPredecessor(),
				node.getEmptyPosition());
	}
	...
	/**
	 * Checks if the action will keep the empty pos in bounds of the state Since it
	 * is a 1D array and the actions treat it like a matrix we need to think about
	 * the mathematical conversion between them. {0,1,2,3,4,5,6,7,8} How it is
	 * stored
	 * 
	 * {{0,1,2}, {3,4,5}, {4,7,8}} How the actions treat it Moving up or down
	 * requires the index to be in bounds for i+-3 Moving left and right requires
	 * the index to be i%3!=0 and i+1%3!=0
	 */
	...
	/**
	 * Generates a child node given a specific action. Returns the child node that
	 * will have the empty tile moved and will have its step cost increased
	 * linearly. Since it is a 1D array and the actions treat it like a matrix we
	 * need to think about the mathematical conversion between them.
	...
	public boolean isSolved() {
		return Arrays.equals(getCurrentState(), GOAL);
	}
}
